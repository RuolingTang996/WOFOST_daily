...
int GetSimInput(char *list) /* 获取模拟输入数据 */
{
    ...
    char output_calendar[MAX_STRING];                                        // 存放输出文件名
    ...
    { // 逐行读取文件
        ...
        sscanf(line, "%s %s %s %s %s %s %d %s %s %s",
               path, cf, sf, mf, site, start, &Emergence, output_daily, output_annual, output_calendar); // 解析行数据
        ...
        if (strlen(output_annual) >= MAX_STRING)
            exit(0);
        if (strlen(output_calendar) >= MAX_STRING)
            exit(0);
        ...
        memset(Grid->output_calendar, '\0', MAX_STRING);
        ...
        strncpy(Grid->output_calendar, output_calendar, strlen(output_calendar)); // Name og output file // 设置输出文件名
        ...
        Grid->file_CO = count;        // number of elements in Grid carousel// 设置Grid轮询中的元素数量
