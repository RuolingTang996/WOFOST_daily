    ...
    while (Meteo)
    {
        ...
        for (Lon = 0; Lon < Meteo->nlon; Lon++)
        {
            for (Lat = 0; Lat < Meteo->nlat; Lat++)
            {
                ...
                // 定义15个特定的(lon,lat)组合
                const double target_lons[] = {116.75, 117.25, 118.25, 115.75, 116.25,
                    116.75, 117.25, 117.75, 118.25, 119.75,
                    120.25, 120.75, 121.75, 122.25, 115.75,
                    116.25, 116.75, 117.25, 118.25, 118.75,
                    119.25, 119.75, 120.25, 120.75, 115.25,
                    115.75, 116.25, 116.75, 117.75, 118.25,
                    118.75, 119.25, 119.75, 120.25, 115.75,
                    116.25, 116.75, 117.25, 117.75, 118.25,
                    118.75, 119.25, 119.75, 114.75, 115.25,
                    115.75, 116.25, 116.75, 117.25, 117.75,
                    118.25, 118.75, 115.25, 115.75, 116.75,
                    117.75, 118.25};

                const double target_lats[] = {37.75, 37.75, 37.75, 37.25, 37.25,
                    37.25, 37.25, 37.25, 37.25, 37.25,
                    37.25, 37.25, 37.25, 37.25, 36.75,
                    36.75, 36.75, 36.75, 36.75, 36.75,
                    36.75, 36.75, 36.75, 36.75, 36.25,
                    36.25, 36.25, 36.25, 36.25, 36.25,
                    36.25, 36.25, 36.25, 36.25, 35.75,
                    35.75, 35.75, 35.75, 35.75, 35.75,
                    35.75, 35.75, 35.75, 35.25, 35.25,
                    35.25, 35.25, 35.25, 35.25, 35.25,
                    35.25, 35.25, 34.75, 34.75, 34.75,
                    34.75, 34.75};


                // 调试输出当前网格点信息
                printf("Checking grid: Lon=%f, Lat=%f\n", Longitude[Lon], Latitude[Lat]);

                // 检查当前(Lon,Lat)是否在目标列表中
                int is_target = 0;
                for (int i = 0; i < 57; i++) {
                    if (fabs(Longitude[Lon] - target_lons[i]) < 0.001 && 
                        fabs(Latitude[Lat] - target_lats[i]) < 0.001) {
                        is_target = 1;
                        printf("MATCH FOUND: Lon=%f, Lat=%f\n", Longitude[Lon], Latitude[Lat]);
                        break;
                    }
                }
                ...
                // 确认匹配后继续执行
                printf("PROCESSING GRID: Lon=%f, Lat=%f\n", Longitude[Lon], Latitude[Lat]);
